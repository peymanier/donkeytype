...
func (m Model) viewHeader() string {
	...
	var headerStyle = newDefaultStyles().headerStyle.Width(availWidth)
	var statStyle = newDefaultStyles().statStyle.Width(availWidth / 4)
...
func (m Model) viewBody() string {
	availWidth := m.width * 4 / 5
	bodyStyle := newDefaultStyles().bodyStyle.Width(availWidth)
